...
def clear_blender():
    ...
    if "Observatory" in bpy.data.collections.keys():
        del_collection(bpy.data.collections["Observatory"])
...
base_path_sun = os.path.join(base_path, 'Textures/')
sun_texture = os.path.join(base_path_sun, '2k_sun.jpg')
star_path_csv = os.path.join(base_path, 'star_paths_03.csv') # Path data for stars in CSV format
...
compass_lable_rotation = 180.0

# Configure compass label text
compass_lable_text = 'SOUTH'
...
def create_material(material_data):
    ...
    for node in material_data['node_data']:
        ...
        for property in material_data['node_data'][node]:
            ...
            if property == 'Image':
                mat_nodes[node].image = material_data['data'][material_data['node_data'][node][property]]
            else:
...
def create_colored_material(name, color_bsdf, color_emission, emission_strenght, mix_frac):
    ...
    return material, shader_emission.inputs['Color'], shader_emission.inputs['Strength']
...
material_sun['nodes'] = {'N_HSV': 'ShaderNodeHueSaturation', 
    'N_Emis': 'ShaderNodeEmission', 
    'N_Tex': 'ShaderNodeTexImage', 
    'N_Out': 'ShaderNodeOutputMaterial'
    }
    
material_sun['links'] = {
    0: {'Out': ['N_Tex', 'Color'], 'In': ['N_HSV', 'Color']},
    1: {'Out': ['N_HSV', 'Color'], 'In': ['N_Emis', 'Color']},
    2: {'Out': ['N_Emis', 'Emission'], 'In': ['N_Out', 'Surface']}
    }

material_sun['node_data'] = {
    'N_Tex': {'Image': 'texture_image'},
    'N_Emis': {'Strength': 'emission_strength'},
    'N_HSV': {'Hue': 'hue', 'Saturation': 'sat', 'Value': 'value'},
    }
    
material_sun['node_coord'] = {
    'N_Tex': (-400, 0),
    'N_HSV': (-200, 0),
    'N_Emis': (0, 0),
    'N_Out': (200, 0)
...
texture_sun = bpy.data.images.load(sun_texture)
    
material_sun['data'] = {
    'texture_image': texture_sun,
    'hue': 0.5,
    'sat': 2.0,
    'value': 0.3,
    'emission_strength': 34.0,
    'material_name': 'sun_texture_material'
    }
    
Materials[material_sun['data']['material_name']] = create_material(material_sun)
...
def create_glossy_colored_material(name, color_glossy_bsdf, roughness):
    ...
    return material, shader_glossy_bsdf.inputs['Color'], shader_glossy_bsdf.inputs['Roughness']
...
def add_hdri_world_shader(hdri_texture, color_world_background):
    ...
    return shader_background_color.inputs['Color']
...
# Add a Sun lightsource to our scene    
add_sun_lightsource()  
...
mix_frac = 0.5
torus_material, _, _ = create_colored_material('torus_material', color_bsdf, color_emission, emission_strength, mix_frac)
...
roughness = 0.146
circle_material, Keyframe_Datapaths['glossy_ground_color'], Keyframe_Datapaths['glossy_ground_roughness'] = create_glossy_colored_material('circle_material', color_glossy_bsdf, roughness)
...
star_blue.hide_render = True
star_red.hide_render = True
star_yellow.hide_render = True

star_blue.hide_viewport = True
star_red.hide_viewport = True
star_yellow.hide_viewport = True
...
emission_strength_star_blue = 34.3
emission_strength_star_red = 27.9
...
if DISPLAY_SUN:
    ...
    star_sun = add_star(scale=0.030, location=(0.7071, -0.7071, 0), name='Star_Sun')
    color_star_sun = hex_to_gamma_corrected_blender_color('#FFD136')
    emission_strength_star_sun = 17.00
    star_sun_material = Materials['sun_texture_material'] #create_colored_emission_material('sun_tstar_material', color_star_sun, emission_strength_star_sun)
    apply_material_to_object(star_sun, star_sun_material)
    star_objects.append(star_sun)
    star_sun.hide_render = True
    star_sun.hide_viewport = True
    
# Add Follow Constraints to Clamshell Observatory Child Objects to Track the Blue Star or Sun
track_object = star_sun

bpy.context.view_layer.objects.active = Objects['Clamshell']
bpy.ops.object.constraint_add(type='LOCKED_TRACK')
bpy.context.object.constraints["Locked Track"].target = track_object
...
bpy.context.object.constraints["Track To"].target = track_object
...
Keyframe_Datapaths['world_background_color'] = add_hdri_world_shader(meadow_hdri_texture, color_world_background) 
...
if DAY:
    ...
    text_base_color = hex_to_gamma_corrected_blender_color('#D7D3D7')
    text_emission_color = hex_to_gamma_corrected_blender_color('#D2CFCE')
    text_emission_strength = 12.2
    text_mix_factor = 0.858
    
else:
    
    text_base_color = slooh_red
    text_emission_color = slooh_red
    text_emission_strength = 1.0
    text_mix_factor = 1.0
...
compass_points_label_material, Keyframe_Datapaths['text_emission_color_value'], Keyframe_Datapaths['text_emission_strenth_value'] = create_colored_material('South_Text_Material', text_base_color, text_emission_color, text_emission_strength, text_mix_factor)
...
number_of_stars = 3
if DISPLAY_SUN: number_of_stars = 4

#print_console(paths_df['X_01'][0])
#print_console(len(paths_df))
...
def create_curve_from_dataframe(dataframe, index):
    ...
    spline.points.add(len(dataframe_star_path) + 1)
    
    #print_console(len(dataframe_star_path))
    
    #print_console(f'X_{(index+1):02}', f'Y_{(index+1):02}', f'Z_{(index+1):02}', len(dataframe_star_path))
    
    x, y, z = dataframe_star_path[f'X_{(index+1):02}'][0], dataframe_star_path[f'Y_{(index+1):02}'][0], dataframe_star_path[f'Z_{(index+1):02}'][0]
    spline.points[0].co = (x, y, z, 1)
    ...
    last_point = len(dataframe_star_path) - 1
    x, y, z = dataframe_star_path[f'X_{(index+1):02}'][last_point], dataframe_star_path[f'Y_{(index+1):02}'][last_point], dataframe_star_path[f'Z_{(index+1):02}'][last_point]
    spline.points[len(dataframe_star_path) + 1].co = (x, y, z, 1)
        
    #print_console(spline.points[len(dataframe_star_path) - 1].co)
...
star_paths_to_display = [3] # [0, 1, 2, 3], range(number_of_stars)
...
for index in range(number_of_stars):
    ...
    #print_console(len(star_path_df))
    
    data_points_number = len(star_path_df[star_path_df[f'DISPLAY_{(index+1):02}']][[f'X_{(index+1):02}', f'Y_{(index+1):02}', f'Z_{(index+1):02}']].copy(index).reset_index(drop=True))
    
    if data_points_number == 0:
        star_path_curves.append(None)
    else:
        star_path_curves.append( create_curve_from_dataframe(star_path_df, index) )
    
for index, curve in enumerate(star_path_curves):
    
    if not(curve == None):
    
        if index in star_paths_to_display:
...
color_star_sun_path = hex_to_gamma_corrected_blender_color('#FFA155')
...
star_emission_strengths = [1.5, 1.8, 3.2]

if DISPLAY_SUN:
    
    star_colors.append(color_star_sun_path)
    stars_materials_names.append('sun_star_path_material')
    star_emission_strengths.append(1.2)

#print_console(star_colors, stars_materials_names, star_emission_strengths)
...
for index in star_paths_to_display:

    # Add circular profile and emission material to star_path curve   
    circel_profile_radius = 0.0003
    #print_console(len(star_path_curves), index, len(star_materials))
...
if COLAB: print('### Beginning Rendering')
...
if COLAB: print(f'No. of Stars to Render: {len(star_objects)}')
...
bpy.context.scene.frame_end = len(paths_df) - 1
        
def clear_animations(objects):
...
def build_animation(dataset, objects, Keyframe_Datapaths, Colors, DAY_NIGHT):
    
    compass_emission_strength_day = 12.2
    compass_emission_strength_night = 1.0
    
    index_dataset = 0
    index_dataset_ = 0
    
    scene.frame_set(index_dataset)
    ...
    for index_objects, object in enumerate(objects):
        ...
        if index_objects == 3:
            ...
            object.location = Vector([dataset[f'X_{(index_objects+1):02}'][index_dataset_], -dataset[f'Y_{(index_objects+1):02}'][index_dataset_], dataset[f'Z_{(index_objects+1):02}'][index_dataset_]])
            ...
            object.keyframe_insert("hide_viewport", frame=0)
            
            DAY = True
            
            if DAY:
                
                    Keyframe_Datapaths['world_background_color'].default_value = Colors['color_world_background_day']
                    Keyframe_Datapaths['world_background_color'].keyframe_insert('default_value', frame = index_dataset)
                    
                    Keyframe_Datapaths['text_emission_color_value'].default_value = Colors['compass_text_background_day']
                    Keyframe_Datapaths['text_emission_color_value'].keyframe_insert('default_value', frame = index_dataset)
                    
                    Keyframe_Datapaths['text_emission_strenth_value'].default_value = compass_emission_strength_day
                    Keyframe_Datapaths['text_emission_strenth_value'].keyframe_insert('default_value', frame = index_dataset)
                    
                    Keyframe_Datapaths['glossy_ground_color'].default_value = Colors['glossy_ground_day']
                    Keyframe_Datapaths['glossy_ground_color'].keyframe_insert('default_value', frame = index_dataset)
                    
                    Keyframe_Datapaths['glossy_ground_roughness'].default_value = 0.146 # 0.161
                    Keyframe_Datapaths['glossy_ground_roughness'].keyframe_insert('default_value', frame = index_dataset)
                    
                    Keyframe_Datapaths['glossy_ground_color'], Keyframe_Datapaths['glossy_ground_roughness']  
    
    # Loop through frames     
    for index_dataset_ in dataset.index: 
        
        index_dataset = index_dataset_ + 1
        ...
        for index_objects, object in enumerate(objects):
            ...
            if index_objects == 3:
                ...
                object.location = Vector([dataset[f'X_{(index_objects+1):02}'][index_dataset_], -dataset[f'Y_{(index_objects+1):02}'][index_dataset_], dataset[f'Z_{(index_objects+1):02}'][index_dataset_]])
                object.keyframe_insert(data_path="location", frame=index_dataset)
                
                # Hide star if below horizon    
                object.hide_render = not( dataset[f'DISPLAY_{(index_objects+1):02}'][index_dataset_] )
                object.keyframe_insert("hide_render", frame=index_dataset)
                
                object.hide_viewport = not( dataset[f'DISPLAY_{(index_objects+1):02}'][index_dataset_] )
                object.keyframe_insert("hide_viewport", frame=index_dataset)
                ...
                if DAY_NIGHT:
                    ...
                    if dataset[f'DAY'][index_dataset]:
                    ...
                    elif dataset[f'NIGHT'][index_dataset]:
                        
                        Keyframe_Datapaths['world_background_color'].default_value = Colors['color_world_background_night']
                        Keyframe_Datapaths['world_background_color'].keyframe_insert('default_value', frame = index_dataset)
                        
                        Keyframe_Datapaths['text_emission_color_value'].default_value = Colors['compass_text_background_night']
                        Keyframe_Datapaths['text_emission_color_value'].keyframe_insert('default_value', frame = index_dataset)
                    
                        Keyframe_Datapaths['text_emission_strenth_value'].default_value = compass_emission_strength_night             
                        Keyframe_Datapaths['text_emission_strenth_value'].keyframe_insert('default_value', frame = index_dataset)
                        
                        Keyframe_Datapaths['glossy_ground_color'].default_value = Colors['glossy_ground_night']              
                        Keyframe_Datapaths['glossy_ground_color'].keyframe_insert('default_value', frame = index_dataset)
                        
                        Keyframe_Datapaths['glossy_ground_roughness'].default_value = 0.146              
                        Keyframe_Datapaths['glossy_ground_roughness'].keyframe_insert('default_value', frame = index_dataset)

    index_dataset_ = len(dataset) - 1
    index_dataset = len(dataset) + 1
...
def render_animation(frame_range, output_dir, output_file_pattern_string = 'render%d.png'):
    ...
    for frame in frame_range:
        ...
        bpy.context.scene.render.filepath = os.path.join(output_dir, (output_file_pattern_string % frame))
        
        # Render Animation
        #print_console(f'Starting Render for Frame: {frame}')
        if COLAB: print(f'Starting Render for Frame: {frame}')
        bpy.ops.render.render(animation=False, write_still=True)
        #print_console(f'Render Complete for Frame: {frame}')
        if COLAB: print(f'Render Complete for Frame: {frame}')

# Clear any existing animations for the star objects
clear_animations(star_objects)

# Create animation keyframes for the stars, location and visibility
Colors = {}

Colors['color_world_background_day'] = hex_to_gamma_corrected_blender_color('#2775EC')
Colors['color_world_background_night'] = color_world_background
Colors['compass_text_background_day'] = hex_to_gamma_corrected_blender_color('#D2CFCE') # ('#E43F15')
Colors['compass_text_background_night'] = slooh_red
Colors['glossy_ground_day'] = color_glossy_bsdf # hex_to_gamma_corrected_blender_color('#4BD64D')
Colors['glossy_ground_night'] = color_glossy_bsdf

DAY_NIGHT = False

build_animation(paths_df, star_objects, Keyframe_Datapaths, Colors, DAY_NIGHT)
...
render_animation(frame_range, base_path_renders, output_file_pattern_string)
