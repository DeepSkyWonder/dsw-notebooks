...
for star_index in range(number_of_stars):
    ...
    star_surf_object.rotation_euler = (0.0, 0.0, 0.0)
    ...
    astrov.import_fbx('corrected_quad_sphere.fbx', star_atmos_name, 1.01)
    star_atmos_object = astrov.Objects[star_atmos_name]
    star_atmos_object.rotation_euler = (0.0, 0.0, 0.0)
...
# To Do
# () Write code to import materials from blend files, and apply to objects
# () Write scene configuration dictionary that is read and processed to produce sceen
# () Write scene animation dictionary that is read and processed to create keyframes
# () Write scene rendering config dictionary that is read an processed to create rendered frames.
# () Remove all ops operators where possible
# () Check object types before performing operations
# () Check modifier already exists before adding
# () Add setup files function and add file_config to AstorViz class
# () Create class for simulation functions and add methods for data transform to vectors and dataframes
# () Workout code for orbit rotations (inc, Omega, omega). Set these up as camera rotations about orbit plane, about barycentre origin
# () Add AU or Arcsec grid to plane below stars
# () Try out sizing stars based of actual radii
# () Write code for loading FBX files with parent object and children objects
# () Read through code and make notes on what need to change
# () Read up on PEP 8 naming conventions
# () Create class for scene setup and object keyframing and animation
