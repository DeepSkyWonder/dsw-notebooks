...
def render_animation(dataset, objects, output_dir, output_file_pattern_string = 'render%d.png'):
    ...
    for step in range(len(dataset)):
        ...
        for index in range(len(objects)):
            ...
            if dataset[f'DISPLAY_{(index+1):02}']:
            
            bpy.context.scene.render.filepath = os.path.join(output_dir, (output_file_pattern_string % step))
            print_data(f'Starting Render for Frame: {step}')
            bpy.ops.render.render(write_still = True)
            print_data(f'Render Complete for Frame: {step}')
...
render_animation(render_df, star_objects[index], base_path_renders, output_file_pattern_string)
