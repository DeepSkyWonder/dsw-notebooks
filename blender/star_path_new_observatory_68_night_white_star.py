...
star_white = add_star(scale=0.015, location=(-0.7071, 0.7071, 0), name='Star_White')

star_objects = [star_yellow, star_red, star_blue, star_white]
...
star_white_material = create_colored_emission_material('white_star_material', color_star_white, emission_strength_star_white)
...
apply_material_to_object(star_white, star_white_material)
...
bpy.context.object.constraints["Locked Track"].target = star_white
...
bpy.context.object.constraints["Track To"].target = star_white
...
number_of_stars = 4
if DISPLAY_SUN: number_of_stars = 5
...
def create_curve_from_dataframe(dataframe, index):
    ...
    print(f'index: {index}')    
    print(f'len star_paths: {len(dataframe_star_path)}')
    print(f'X_{(index+1):02}')
    print(dataframe_star_path[f'X_{(index+1):02}'][0])
...
star_colors = [color_star_yellow, color_star_red, color_star_blue, color_star_white]
stars_materials_names = ['yellow_star_path_material', 'red_star_path_material', 'blue_star_path_material', 'white_star_path_material']
star_emission_strengths = [1.5, 1.8, 3.2, 1.3]

if DISPLAY_SUN:
    
    star_colors.append(color_star_sun)
...
for index in range(number_of_stars):
    ...
    print(f'column index: X_{(index+1):02}')
    print(len(star_objects))
...
def render_animation(start_frame, end_frame, output_dir, output_file_pattern_string = 'render%d.png'):
...
render_animation(start_frame, end_frame, base_path_renders, output_file_pattern_string)
