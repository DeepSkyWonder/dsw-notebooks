...
def render_animation(dataset, objects, output_dir, output_file_pattern_string = 'render%d.png'):
    ...
    for index_dataset in dataset.index: 
        ...
        for index_objects, object in enumerate(objects):
            ...
            object.location = Vector([dataset[f'X_{(index_objects+1):02}'][index_dataset], -dataset[f'Y_{(index_objects+1):02}'][index_dataset], dataset[f'Y_{(index_objects+1):02}'][index_dataset]])
            
            # Hide star if below horizon    
            object.hide_render = not( dataset[f'DISPLAY_{(index_objects+1):02}'] )
            object.hide_viewport = not( dataset[f'DISPLAY_{(index_objects+1):02}'] )
        ...
        print_console(f'Render Complete for Frame: {steindex_datasetp}')
