...
star_path_data = os.path.join(base_path, 'star_paths.csv') # Path data for stars in CSV format
...
path_df = pd.read_csv(star_paths.csv)
...
def create_curve_from_dataframe(dataframe, index):
    ...
    dataframe_star_path = star_path_data[star_path_data[f'DISPLAY_{(index+1):02}']][[f'X_{(index+1):02}', f'Y_{(index+1):02}', f'Z_{(index+1):02}']].copy(index).reset_index(drop=True)
...
for index in range(3):
    ...
    star_path_df = paths_df[[[f'X_{(index+1):02}'], [f'Y_{(index+1):02}'], [f'Z_{(index+1):02}'], [f'DISPLAY_{(index+1):02}']]]
...
add_circle_profile_and_material_to_curve(star_path_object_01, circel_profile_radius, blue_star_path_material)
