...
def add_torus(major_radius=1.0, minor_radius=0.002, location=(0, 0, 0)):
    ...
    bpy.ops.mesh.primitive_torus_add(align='WORLD', location=loctaion, rotation=(0, 0, 0), major_segments=100, minor_segments=50, major_radius=major_radius, minor_radius=minor_radius, abso_major_rad=1.25, abso_minor_rad=0.75)
...
def add_cirle(radius=1.0, location=(0.0, 0.0, 0.0005), fill_type='TRI'):
...
def add_hdri_world_shader(hdri_texture, color_world_background):
    ...
    shader_background_color.color = color_world_background
    ...
    link = links.new(input_lightpath.outputs["Is Camera Ray"], shader_mix.inputs["Fac"])
    link = links.new(shader_environment.outputs["Color"], shader_background_texture.inputs["Color"])
    link = links.new(shader_background_texture.outputs["Background"], shader_mix.inputs[1]) 
    link = links.new(shader_background_color.outputs["Background"], shader_mix.inputs[2]) 
    link = links.new(shader_mix.outputs["Shader"], output_world.inputs["Surface"]) 
    
    

# Notes - To Do
# Add new glossy shader for circle
# Add World HDRI Shader
# Alter observatory shader
# Check ring shader
# Add additional stars
# Correct Normals on Hemisphere
# Add text for south and east (possibly north and west)
# Add function to turn on/off text element during render
# Check any material changes to any of the objects
...
color_emission = hex_to_blender_color('#435973')
color_bsdf = hex_to_blender_color('#435973')
emission_strenght = 1.7
mix_frac = 0.5
torus_material = create_colored_material('torus_material', color_bsdf, color_emission, emission_strenght, mix_frac)
...
circle = add_cirle(radius=1.0, location=(0.0, 0.0, 0.0005), fill_type='TRI')

# Create material for (filled circle) disk
color_glossy_bsdf = hex_to_blender_color('#364E62')
roughness = 0.146
...
star_red = add_star(scale=0.015, location=(0.7071, 0.7071, 1), name='Star_Red')
star_yellow = add_star(scale=0.015, location=(0.7071, -0.7071, 1), name='Star_Yellow')

# Create material for stars
color_star_blue = hex_to_blender_color('#1A5AFF')
color_star_red = hex_to_blender_color('#FF4C2E')
color_star_yellow = hex_to_blender_color('#FFEC3C')
...
star_red_material = create_colored_emission_material('blue_star_material', color_star_red, emission_strength_star_red)
star_yellow_material = create_colored_emission_material('blue_star_material', emission_strength_star_yellow, emission_strength_star_yellow)
...
color_world_background = hex_to_blender_color('#202E40')
